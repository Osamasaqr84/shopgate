...
public class AllFavProductsAdapter extends RecyclerView.Adapter<AllFavProductsAdapter.ViewHolder> {
        ...
        try {
            if (productsbysubcats.get(position).getProduct().getProductphotos().size() > 0)
                Glide.with(context.getApplicationContext())
                        .load(productsbysubcats.get(position).getProduct().getProductphotos().get(0).getPhoto())
        ...
        holder.addtocart.setOnClickListener(v -> {
            if (PreferenceHelper.getUserId() > 0) {
                if (PreferenceHelper.retriveCartItemsValue() != null) {
                    if (!PreferenceHelper.retriveCartItemsValue().contains(String.valueOf(productsbysubcats.get(position).getProduct().getProductsizes().get(0).getId()))) {
                        PreferenceHelper.addItemtoCart(productsbysubcats.get(position).getProduct().getProductsizes().get(0).getId());
    ...
    class ViewHolder extends RecyclerView.ViewHolder {
        ...
        private TextView name, rateCount, amount, price,addtocart;
